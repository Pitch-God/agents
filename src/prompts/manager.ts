...
import { input2, input } from "../lib/examples/reply.js";
  ...
  z.object({
    isTranslationRequired: z.object({
      isTranslationRequired: z
        .boolean()
        .describe(
          "The translation is required if the conversation is not in english",
        ),
      language: z
        .string()
        .optional()
    ...
    schedule: z.object({
      ...
      isRequired: z
        .boolean()
        .describe(
          "If the recipient has not requested a meeting and but it is okay to ask for one",
        ),
    }),
    isRetrievalRequired: z.boolean().describe("isRetrievalRequired"),
